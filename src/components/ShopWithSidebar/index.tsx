...
const ShopWithSidebar = () => {
  ...
  const [selectedPrice, setSelectedPrice] = useState({ from: 0, to: 500 });
  ...
  useEffect(() => {
    ...
    const handleCategoryFilter = (e: any) => {
      console.log("Event received in ShopWithSidebar:", e.detail);
    ...
    console.log("Adding event listener for categoryFilter");
    window.addEventListener('categoryFilter', handleCategoryFilter);

    return () => {
      console.log("Removing event listener for categoryFilter");
  ...
  const filteredData = useMemo(() => {
    ...
    if (selectedCategories.length > 0) {
      result = result.filter((p) => {
        ...
        return selectedCategories.some((name) => name.toLowerCase() === categoryName);
      });
    }
    ...
    if (selectedPrice) {
      result = result.filter(
        (p) => p.price >= selectedPrice.from && p.price <= selectedPrice.to
      );
    }
    ...
    return result;
  }, [products, categories, selectedCategories, selectedPrice, searchQuery]);
  ...
  // Build dynamic categories with product counts from database
  ...
  // Convert database product to match the expected format for grid/list items
  const convertProduct = (product: Product) => {
    const category = categories.find(c => c.id === product.category_id);

    // Handle images: prioritize images array, then image_url, then empty array
    ...
    return {
      id: product.id,
      title: product.title,
      reviews: 15, // You can add reviews field to database later
      category: category?.name || "Uncategorized",
      price: product.price,
      discountedPrice: product.price, // You can add discount field to database later
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-[#f3f4f6]">
        <div className="flex flex-col items-center gap-4">
          <div className="w-16 h-16 border-4 border-t-4 rounded-full animate-spin" style={{ borderColor: "#E0E7FF", borderTopColor: "#3B82F6" }}></div>
          <p className="text-sm font-semibold animate-pulse" style={{ color: "#64748B" }}>Loading products...</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <>
      <Breadcrumb
        ...
        pages={["shop"]}
      />
      <section id="products-section" className="overflow-hidden relative pb-20 bg-[#f3f4f6]">
        <div
          ref={gridRef}
                  ...
                  <div className="bg-white shadow-1 rounded-lg py-4 px-5">
                    <div className="flex items-center justify-between">
                      ...
                      <button
                        ...
                        onClick={() => {
                          setSelectedCategories([]);
                          setSelectedPrice({ from: 0, to: 500 });
                          setSearchQuery("");
                          setCurrentPage(1);
                        }}
                ...
                {paginatedData.length > 0 ? (
                  paginatedData.map((product, idx) => {
                    ...
                    return productStyle === "grid" ? (
                      <SingleGridItem item={convertedProduct} key={product.id} />
                    ) : (
                      <SingleListItem item={convertedProduct} key={product.id} />
                    );
                  })
                ) : (
                  <div className="col-span-full text-center py-12">
                    ...
                    <button
                      ...
                      className="mt-4 px-6 py-2 bg-blue text-white rounded-lg hover:bg-opacity-90 transition-all"
                    >
                      Clear Filters
                    </button>
                  </div>
