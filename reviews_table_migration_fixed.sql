-- ===================================================================
-- Reviews Table Migration - FIXED VERSION
-- ===================================================================
-- This fixes the foreign key relationship to work with public.users table

-- Drop existing reviews table if it exists
DROP TABLE IF EXISTS reviews CASCADE;

-- Create reviews table with correct foreign key to public.users
CREATE TABLE reviews (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  product_id UUID NOT NULL REFERENCES products(id) ON DELETE CASCADE,
  user_id UUID NOT NULL REFERENCES users(id) ON DELETE CASCADE,  -- References public.users, not auth.users
...
-- 2. SELECT: Super admins and admins can view all reviews (including hidden ones)
...
-- 5. UPDATE: Admins can update any review (for hiding/showing)
...
-- 6. DELETE: Admins can delete any review
...
-- - Fixed foreign key to reference public.users instead of auth.users
-- - Average rating now rounds to 1 decimal place (e.g., 4.5)
-- - Trigger now handles DELETE operations to update ratings when reviews are deleted
-- - Both admin and super_admin can manage reviews
